...
| 127         | 126            | 125-124         | 123-108 | 107-92  | 91-90   | 89-88   | 87-60      | 59-32        | 31-8 | 7       | 6       | 5                | 4                | 3-0    |
| work policy | monitor policy | encoding scheme | reply   | origin  | reply   | origin  | work mode  | monitor mode |      | ingress | egress  | source work mode | target work mode | round  |
| action drop | action drop    | (0x11)          | in_port | in_port | source  | source  | flow id    | flow id      |      | treated | treated | action drop      | action drop      | number |
...
type DecodedMicroSegmentationConntrackLabels struct {
	...
	EgressTreated            bool           `json:"egress_treated"`               // 1 bit
	IngressTreated           bool           `json:"ingress_treated"`              // 1 bit
...
var (
	...
	// 31-8 not used
	//
	IngressTreatedLength           = uint8(1)
	IngressTreatedShift            = uint8(7)
	IngressTreatedMask             = numeric.Mask(IngressTreatedLength).ShiftLeft(IngressTreatedShift)
	EgressTreatedLength            = uint8(1)
	EgressTreatedShift             = uint8(6)
	EgressTreatedMask              = numeric.Mask(EgressTreatedLength).ShiftLeft(EgressTreatedShift)
...
func DecodeMicroSegmentation(labels numeric.Uint128) (DecodedMicroSegmentationConntrackLabels, error) {
	...
	decoded.EgressTreated = labels.And(EgressTreatedMask).ShiftRight(EgressTreatedShift).Low != 0
	decoded.IngressTreated = labels.And(IngressTreatedMask).ShiftRight(IngressTreatedShift).Low != 0
